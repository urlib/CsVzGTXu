...
{
    return (ll)(a.x - p[_t].x) * (b.y - p[_t].y) < (ll)(b.x - p[_t].x) * (a.y - p[_t].y);
}
...
void func(int x, int fa, int l, int r)
{
    ...
    sort(p + 1 + l, p + 1 + r, cmp);
    ans[p[_t].id] = x;
    for (auto v : e[x])
    {
        if (v != fa)
        {
            func(v, x, l + 1, l + size[v]);
            l += size[v];
        }
    }
}
