...
inline ll hush(int A, int B)
{
    return ((ll)A << HUSH) + (ll)B;
}

unordered_map<ll, ll> mp[N];
ll dfs(int now, int A, int B)
{
    if (A == _and && B == _and)
    {
        return 1ll << (n - now + 1);
    ...
    if (mp[now].find(hush(A, B)) != mp[now].end())
    {
        return mp[now][hush(A, B)];
    }
    else
    {
        return mp[now][hush(A, B)] = dfs(now + 1, A & a[now], B) + dfs(now + 1, A, B & a[now]);
    }
}
...
int main()
{
    ...
    io::writeln(dfs(1, (1 << HUSH) - 1, (1 << HUSH) - 1) - (_and == (1 << HUSH) - 1 ? 2 : 0));
}